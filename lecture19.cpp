...
class BST
{
	private:
		...
		int getMinInSubtree(Node*); //Node* is root of subtree where we're looking
		int getMaxInSubtree(Node*);
		//THE CHANGES ARE HERREEEEEEEEEEEEEE
		void printNodesInOrder(Node*); //Node* where you start from
...
//min will always be on farthest left
int BST::getMinInSubtree(Node* startNode)
{
	...
	{
		return -1; //some number
	}
}

//same code, just with right since max will be on right
...
bool BST::deleteNode(Node* startNode, int itemToFind) //questionable code
{
	...
	if (target != nullptr)
	{
		...
		if (target->right != nullptr)
		{
			...
			//just looking at values and putting them in the right spot
			...
			delete minRightNode; //not actually deleting, just replacing values
			target->id = minRight;
			return true;
		}
	}
